...
public class SeatpdfSrvc {
	...
	public ByteArrayOutputStream pdfcontent(Long busschid)
	{
		...
		try
		{
			...
			{
				Paragraph p1=new Paragraph("Seat ".concat(i.toString()).concat(":"));
				document.add(p1);
					...
					if(occ.equals("Y"))
					{
						...
						else
						{
							...
							else
							{
								...
								String temp=loc1.concat(" - ").concat(loc2).concat("  ->  ").concat(cstobj.getCstmrname()).concat("[").concat(cstobj.getMobileno()).concat("]");
